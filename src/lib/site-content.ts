...
export type ProjectItem = (typeof seededProjects)[number] & {
  info?: {
    scale?: string;
    location?: string;
...
async function seedCollections() {
  ...
  const [siteContentCount, projectCount, reelCount] = await Promise.all([
    SiteContent.countDocuments({ key: 'global' }),
    Project.countDocuments(),
    Reel.countDocuments()
  ]);

  if (siteContentCount === 0) {
    await SiteContent.create({ key: 'global', data: defaultSiteContent });
  }
  ...
  if (reelCount === 0) {
    await Reel.insertMany(seededReels);
  }
}

export const ensureSeededContent = cache(async () => {
...
export const getProjects = cache(async (): Promise<ProjectItem[]> => {
  await ensureSeededContent();
  const docs = await Project.find().sort({ createdAt: 1 }).lean<ProjectItem[]>();
  return docs;
});

export const getProjectBySlugFromDb = cache(async (slug: string): Promise<ProjectItem | null> => {
  await ensureSeededContent();
  const doc = await Project.findOne({ slug }).lean<ProjectItem | null>();
  return doc;
});

export const getReels = cache(async (): Promise<ReelItem[]> => {
  await ensureSeededContent();
  const docs = await Reel.find().sort({ createdAt: 1 }).lean<ReelItem[]>();
  return docs;
});
